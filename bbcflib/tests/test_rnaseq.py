...
class Test_Rnaseq(unittest2.TestCase):
	...
	self.counts = numpy.array([[12,12,12,12,12,12],[3,3,3,3,3,3]], dtype=numpy.float32)
	self.dexons = dict(zip(["e1","e2","e3","e4","e5","e"],zip(*self.counts)))
	self.gene_ids = ["g1","g2"]
	self.exon_mapping = {
	...
	self.assertListEqual(list(texp["t"]), [12,3])
