...
def get_project(mail, key, project_key, serv_url=default_url):
    ...
    :param mail : login to TEQUILA.
    :param key : an user-specific key (ask for it to a GDV admin).
...
def new_project(mail, key, name, assembly_id, serv_url=default_url):
    ...
    :param mail: login to TEQUILA.
    :param key: a user-specific key (ask for it to a GDV admin).
...
def single_track(mail, key, serv_url=default_url,
    ...
    :param name: name of the project.
    :param assembly_id: the assembly identifier in GenRep (must be BBCF_VALID).
        Optional if a project_id is specified.
    ...
    :param delfile: If true and file coming from fsys, the original file will be removed after job success.
    :rtype: JSON
    '''
...
def multiple_tracks(mail, key, serv_url=default_url,
    ...
    :param tracknames: a list of file names, in the same order than the files uploaded.
