...
#                       }
        ...
        ex.add(output,description=set_file_descr("meanScorePerFeature_"+density_name+".sql",group=density_name,step="norm_counts_per_frag",type="sql",view="admin",ucsc='1'))
     #  'output_location':wd,
#	print(gMiner_job)
...
##	ex.add(countsPerFragFile,description="none:bed:meanScorePerFeature_"+density_name+".bed (template) [group:"+str(grpId)+",step:"+str(step)+",type:template,view:admin]")
	ex.add(countsPerFragFile,description=set_file_descr("meanScorePerFeature_"+density_name+".bed",group=density_name,step="density",type="bed"))
...
##			associate_to_filename=countsPerFragFile, template="%s"+".bed")
	...
	ex.add(res,description=set_file_descr("res_segToFrag_"+density_name+".bedGraph",group=density_name,step="norm_counts_per_frag",type="bedGraph",view="admin",comment="rough"))
#	ex.add(res,description="none:res_segToFrag_"+density_name+" (rough) [group:"+str(grpId)+",step:"+str(step)+",type:bedGraph,view:admin]")
	...
	ex.add(resBedGraph,description=set_file_descr("res_segToFrag_"+density_name+".bedGraph",group=density_name,step="norm_counts_per_frag",type="bedGraph",view="admin",comment="bedGraph non-sorted"))
	ex.add(resBedGraph_all,description=set_file_descr("res_segToFrag_"+density_name+"_all_nonSorted.bedGraph",group=density_name,step="norm_counts_per_frag",type="bedGraph",view="admin",comment="all informative frags - null included - bedGraph non-sorted"))
	resBedGraph_all=call_sortOnCoord(ex,resBedGraph_all,via=via)
	ex.add(resBedGraph_all,description=set_file_descr("res_segToFrag_"+density_name+"_all.bedGraph",group=density_name,step="norm_counts_per_frag",type="bedGraph",view="admin",comment="all informative frags - null included -sorted bedGraph"))
...
##	ex.add(sortedBedGraph,description="none:res_segToFrag"+denstiy_name+" (template) [group:"+str(grpId)+",step:"+str(step)+",type:template,view:admin]")
	ex.add(sortedBedGraph,description=set_file_descr("res_segToFrag_"+density_name+".bedGraph",group=density_name,step="norm_counts_per_frag",type="bedGraph",comment="bedGraph sorted",uscc='1'))
...
##	ex.add(sortedBedGraph_sql,description="none:res_segToFrag_"+density_name+".sql (template) [group:"+str(grpId)+"step:"+str(step)+",type:template,view:admin]")
	ex.add(sortedBedGraph_sql+".sql",description=set_file_descr("res_segToFrag_"+density_name+".sql",group=density_name,step="norm_counts_per_frag",type="sql",view="admin",comment="bedGraph sorted"))
...
##				associate_to_filename=mapseq_wig, template="%s"+".wig")
			...
			ex.add(mapseq_files[gid][rid]['wig']['merged'],description=set_file_descr("density_file_"+mapseq_files[gid][rid]['libname']+".sql",group=group['name'],step="density",type="sql",view='admin'))
#		        ex.add(mapseq_files[gid][rid]['wig']['merged'],description="sql:density_file_"+mapseq_files[gid][rid]['libname']+" [group:"+str(grpId)+",step:"+str(step)+",type:sql]"  )
 ...
 #                       #        associate_to_filename=mapseq_files[gid][rid]['wig']['merged'], template='%s'+'.sql')	
			...
			ex.add(outputfile_afterProfileCorrection,description=set_file_descr("res_segToFrag_"+mapseq_files[gid][rid]['libname']+"_profileCorrected_smoothed_"+nFragsPerWin+"FragsPerWin.bedGraph",group=group['name'],step="smoothing",type="bedGraph",comment="smoothed data, after profile correction",ucsc='1'))
		grpId += 1
		step=0
	return processed
