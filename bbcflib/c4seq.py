...
@program
#--args segToFrag_PFBM8_Sp9_all.bedGraph normFile.bedGraph chr2:73114371-73115873 1000000 chr2:73112371-73118419
def normFrags( inputFile, outputFile, baitCoord, sizeExt, regToExclude, script_path=''):
    args = _RCMD(script_path,"normFrags.R")+[inputFile,outputFile,baitCoord,str(sizeExt),regToExclude]
    print(args)
    return {'arguments': args, 'return_value': None}

@program
#mergeRep.R --args file1;file2;file3 mergedfile.bedGraph 4 chr2:73911294-73916104
def mergeRep(inputFiles, outputFile, regToExclude, idCol="4", script_path=''):
    #time.sleep(20)
    args = _RCMD(script_path,"mergeRep.R")+[inputFiles,outputFile,idCol,regToExclude]
    print(args)
    return {'arguments': args, 'return_value': None}

@program
#combineTracks.R --args $fragsFiles combinedFile.txt $names 4,5 $reffile
def makeTable(inputFiles, outputFile, names, idCols="4", defVal=0, script_path=''):
    #time.sleep(30)
    args = _RCMD(script_path,"combineTracks.R")+[inputFiles,outputFile,names,idCols,str(defVal)]
    print(args)
    return {'arguments': args, 'return_value': None}
...
#reffile='/archive/epfl/bbcf/data/DubouleDaan/library_Nla_30bps/library_Nla_30bps_segmentInfos.bed'
    ...
    for gid, group in job.groups.iteritems():
        ...
        for rid,run in group['runs'].iteritems():
            ...
            print "call normFrags: infiles="+resfile+", normfile="+normfile+"baitCoord="+primers_dict[group['name']]['baitcoord']+", sizeExt=1000000, regToExclude="+regToExclude[gid]+"\n"
            futures_norm[gid][rid] = normFrags.nonblocking( ex, resfile, normfile, baitCoord=primers_dict[group['name']]['baitcoord'], sizeExt=1000000, regToExclude=regToExclude[gid], script_path=script_path, via=via )
    ...
    for gid, group in job.groups.iteritems():
        ...
        futures_merged[gid] = mergeRep.nonblocking( ex, ",".join([res_rid for rid,res_rid in processed['4cseq']['norm'][gid].iteritems()]), mergefile, regToExclude[gid], script_path=script_path, via=via , memory= 8)
    ...
    for gid, group in job.groups.iteritems():
        ...
        futures_profcor_merged[gid] = mergeRep.nonblocking( ex, ",".join(pcfiles), mergefile, regToExclude[gid], script_path=script_path, via=via , memory= 8)
    ...
    for gid, group in job.groups.iteritems():
        ...
        futures_smoothed[gid] = ( smoothFragFile.nonblocking( ex, processed['4cseq']['norm_grp'][gid], nFragsPerWin, group['name'],
        ...
        processed['4cseq']['smooth_grp'][gid] = [file1,file2] #[smoothed file before PC, smoothed file after PC]
    ...
    for gid, group in job.groups.iteritems():
        ...
        if run_domainogram[gid]:
            ...
            if before_profile_correction[gid]:
               futures_domainograms[gid] = runDomainogram.nonblocking( ex, processed['4cseq']['norm_grp'][gid], grName, regCoord=regCoord,
                                                                    script_path=script_path, via=via, memory=15 )
            else:
                futures_domainograms[gid] = runDomainogram.nonblocking( ex, processed['4cseq']['profileCorrection_grp'][gid], grName, regCoord=regCoord.split(':')[0],
                                                                        skip=1, script_path=script_path, via=via, memory=15 )
...
############### prepare tables for global results
    ...
    print(",".join(allNames))
    touch(ex,tablePC)
    #for f in allFiles: copyfile(f, "/scratch/cluster/monthly/mleleu/tmp/"+f)
    futures_tables = (makeTable.nonblocking(ex, ",".join(allFiles), tablePC, ",".join(allNames), idCols="4,5", script_path=script_path, via=via, memory=8 ), )
    ...
    for gid, group in job.groups.iteritems():
        ...
        allFiles += [ processed['4cseq']['norm_grp'][gid], processed['4cseq']['smooth_grp'][gid][0] ]

    tableSmoothed_grp=unique_filename_in()+".txt"
    touch(ex,tableSmoothed_grp)
    futures_tables += (makeTable.nonblocking(ex, ",".join(allFiles), tableSmoothed_grp, ",".join(allNames), idCols="4", script_path=script_path, via=via, memory=8 ), )

    allNames=[]
    allFiles=[]
    for gid, group in job.groups.iteritems():
        allNames += [ group['name']+"_normPC", group['name']+"_smoothedPC" ]
        allFiles += [ processed['4cseq']['profileCorrection_grp'][gid], processed['4cseq']['smooth_grp'][gid][1] ]

    tableSmoothedPC_grp=unique_filename_in()+".txt"
    touch(ex,tableSmoothedPC_grp)
    futures_tables += (makeTable.nonblocking(ex, ",".join(allFiles), tableSmoothedPC_grp, ",".join(allNames), idCols="4", script_path=script_path, via=via, memory=8 ), )

    ## combine BRICKS2Frags files
    allNames=[]
    allFiles=[]
    for gid, f in futures_BRICKS2Frags.iteritems():
        f.wait()
        allNames += [ job.groups[gid]['name']+"_BRICKSpval" ]
        allFiles += [ processed['4cseq']['bricks2frags'][gid] ]

    tableBRICKS2Frags = unique_filename_in()+".txt"
    touch(ex,tableBRICKS2Frags)
    futures_tables += (makeTable.nonblocking(ex, ",".join(allFiles), tableBRICKS2Frags, ",".join(allNames), idCols="4", defVal="NA", script_path=script_path, via=via, memory=8 ), )
...
################ Add everything to minilims below!
    ...
    for gid in processed['4cseq']['density_files'].keys():
        for rid, sql in processed['4cseq']['density_files'][gid].iteritems():
            fname = "density_file_"+job.groups[gid]['name']+"_merged_"+str(rid)
    ...
    for gid in processed['4cseq']['countsPerFrag'].keys():
        for rid, resfiles in processed['4cseq']['countsPerFrag'][gid].iteritems():
            fname = "meanScorePerFeature_"+job.groups[gid]['name']+"_"+str(rid)
    ...
    for gid, resfile in processed['4cseq']['norm_grp'].iteritems():
        ...
        gzipfile(ex,resfile)
        ex.add( resfile+".gz", description=set_file_descr( fname+".gz", groupId=gid,step=step, type="bedGraph",ucsc='1'))
    ...
    for gid, dict_gid in processed['4cseq']['norm'].iteritems():
        for rid, resfile in dict_gid.iteritems():
            fname = "normalised_scorePerFeature_"+job.groups[gid]['name']+"_"+str(rid)
            gzipfile(ex,resfile)
            ex.add(resfile+".gz",
    ...
    for gid, dict_gid in processed['4cseq']['profileCorrection'].iteritems():
        for rid, resfiles in dict_gid.iteritems():
            ...
            fname = "segToFrag_"+job.groups[gid]['name']+"_profileCorrected_"+str(rid)
            gzipfile(ex,profileCorrectedFile)
            ex.add( profileCorrectedFile+".gz",
                    description=set_file_descr(fname+".bedGraph.gz",groupId=gid,step=step,type="bedGraph",ucsc='1',gdv='1'))
